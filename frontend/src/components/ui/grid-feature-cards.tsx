...
export function FeatureCard({ feature, className, ...props }: FeatureCardProps) {
	...
	return (
		<div className={cn('relative overflow-hidden p-6 bg-black', className)} {...props}>
			<div className="pointer-events-none absolute top-0 left-1/2 -mt-2 -ml-20 h-full w-full [mask-image:linear-gradient(white,transparent)]">
				<div className="absolute inset-0 bg-gradient-to-r from-white/5 to-white/[0.01] [mask-image:radial-gradient(farthest-side_at_top,white,transparent)] opacity-100">
					<GridPattern
						...
						y="4"
						squares={p}
						className="fill-white/5 stroke-white/20 absolute inset-0 h-full w-full mix-blend-overlay"
					/>
				</div>
			</div>
			<feature.icon className="text-white/70 size-6" strokeWidth={1} aria-hidden />
			<h3 className={`mt-10 text-sm md:text-base ${feature.highlight ? "text-orange-400" : "text-white"}`}>{feature.title}</h3>
